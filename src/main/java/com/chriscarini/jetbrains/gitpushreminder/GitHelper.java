...
{
   final Set<RepositoryAndBranch> allBranchesWithUnpushedCommits =
    gitRepository.getBranches().getLocalBranches()
     .stream()
     .filter(
      gitLocalBranch -> isLocalBranchWithUnpushedCommits(project, gitRepository, gitLocalBranch))
...
)
{
   ...
   return isLocalBranchWithUnpushedCommits(project, gitRepository, currentBranch) ?
           Set.of(new RepositoryAndBranch(gitRepository, currentBranch)) :
...
)
{
   ...
   final boolean outgoingCommits = GitBranchIncomingOutgoingManager.getInstance(project)
                                    .hasOutgoingFor(gitRepository, currentBranchName);
